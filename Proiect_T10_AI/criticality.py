...
def analyze_module(source_path: str | Path) -> dict[str, MethodMetrics]:
    ...
    def _process_function(func_node: ast.FunctionDef | ast.AsyncFunctionDef):
        an = _FunctionAnalyzer(func_node.name, func_node.lineno)
        an.visit(func_node)
        score = float(
            2.5 * an.cyclomatic
            + 1.8 * an.branch_nodes
        ...
        out[func_node.name] = MethodMetrics(
            name=func_node.name,
    ...
    for node in tree.body:
        # 1. Caută metode în interiorul claselor
        if isinstance(node, ast.ClassDef):
            for item in node.body:
                if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    _process_function(item)
                    
        # 2. Caută funcții definite direct la nivel de modul
